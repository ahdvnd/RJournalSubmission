...
Massive Open Online Courses (MOOCs), research on MOOCs is critical to
understand current and future trends in education. Although research in
the field has rapidly grown in recent years, one of the main challenges
facing researchers remains to be the complexity and messiness of the
data. Therefore, it is imperative to provide tools that pave the way for
more research on the new subject of MOOCs. This paper introduces a
package called \pkg{crsra} based on the statistical software R to help
clean and analyze massive loads of data from the Coursera MOOCs. The
advantages of the package are as follows: a) faster loading of data for
analysis, b) an efficient method for combining data from multiple
courses and even across institutions, and c) provision of a set of
functions for analyzing student behaviors.
}
...
gigabytes. We used the \pkg{crsra} package to import the data on all the
...
We can use the function \texttt{crsra\_gradesummary()} to calculate the
average student grade for the courses in the data import. By using the
argument \texttt{groupby} we can calculate average grades for different
groups of the students by factors such as gender, education, student
status, employment status, and country. For instance, the following
analysis returns the average overall course grade for male and female
learners in the course \emph{The Data Scientist's Toolbox}. The result
points to the fact that female learners' grades are on average 6 points
lower on a 100 scale than male learners' grades.
...
the \pkg{crsra} package to analyze student progress and pace in one of
